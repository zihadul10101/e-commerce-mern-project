...
const userForgetPasswordByEmail = async (email) => {
  ...
  try {
    ...
    const activationPath = `/users/reset-password/${token}`; // Activation path with token
    const emailData = {
      email,
      ...
      html: `
        ...
        <a href="${baseUrl + activationPath}"
         target="_blank">
          your forget password account</a>
...
module.exports = {
  handleUserAction, findUsers, findUserById, deleteUserById, updateUserById,
